# motionlab/serve.py: the gesture sidecar Hannah talks to on :8005 (`python -m motionlab.serve`).
#
...
# MOTIONLAB_RUNS: the folder holding the weights (runs/vae/latest.pt, runs/flow/latest.pt); Hannah points it at
# backend/sidecar/gestures/runs. VAE_CKPT / FLOW_CKPT override each file.
_RUNS = os.environ.get("MOTIONLAB_RUNS", "runs")
VAE_CKPT = os.environ.get("VAE_CKPT", os.path.join(_RUNS, "vae", "latest.pt"))
FLOW_CKPT = os.environ.get("FLOW_CKPT", os.path.join(_RUNS, "flow", "latest.pt"))
...
_LIB_DIR = Path(__file__).resolve().parent / "assets" / "action_library"
...
def main() -> None:
    """`python -m motionlab.serve [--host 127.0.0.1] [--port 8005]`"""
    import argparse
    import uvicorn
    ap = argparse.ArgumentParser(description="Hannah gesture sidecar (text -> motion)")
